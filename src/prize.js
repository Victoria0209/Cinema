...
let nameField;
let emailField;
if ($('#prize_pop-up input[name="feedback_name"]').length>=1) {
    
    nameField = document.querySelector('#prize_pop-up input[name="feedback_name"]').parentNode;
    emailField = document.querySelector('#prize_pop-up input[name="email"]').parentNode;
}
...
let nameFieldUtiles;
let emailFieldUtiles;

if ($('#prize_pop-up input[name="feedback_name"]').length>=1) {
    selectPrize.addEventListener('change', () => {
        selectPrize.classList.add('input_select_selected');
    });
     nameFieldUtiles = initializeField(nameField);

     emailFieldUtiles = initializeField(emailField);
